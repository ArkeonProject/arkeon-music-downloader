...
from ..db.database import ensure_database_schema
from . import routes
import threading
...
# Create/update database schema on startup. This keeps existing SQLite Docker
# volumes compatible without requiring an external migration step.
...
def _sync_existing_sources_to_navidrome():
    ...
    try:
        ...
        for source in sources:
            ...
            if playlist_id:
                logger.info(
                    f"Ensured Navidrome playlist '{source.name}' with ID: {playlist_id}"
                )
            ...
            if playlist_id:
                ...
                routes._sync_existing_tracks_to_navidrome(
                    source.id, playlist_id, source.name
                )
...
async def lifespan(app: FastAPI):
    ...
    download_path = os.getenv(
        "DOWNLOAD_PATH", str(Path(__file__).parent.parent.parent.parent / "downloads")
    )
    interval = int(os.getenv("OBSERVER_INTERVAL_MS", "60000"))

    # Parse sync settings from environment
    enable_sync_deletions = (
        str(os.getenv("ENABLE_SYNC_DELETIONS", "true")).lower() == "true"
    )
    ...
    default_cookies = os.getenv(
        "COOKIES_PATH",
        str(Path(__file__).parent.parent.parent.parent / "data" / "cookies.txt"),
    )
