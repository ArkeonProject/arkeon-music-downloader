...
TEST_PLAYLIST_URL = "https://music.youtube.com/playlist?list=PLH_LluK-ePJ__EFdCYCMfPy4oZjDfZF2k"
...
def test_integration_full_workflow():
    ...
    print("   - Ve a: https://music.youtube.com/playlist?list=PLH_LluK-ePJ__EFdCYCMfPy4oZjDfZF2k")
    ...
    if len(videos) == 0:
        print("   ⚠️  No se detectaron canciones. Asegúrate de agregar canciones a la playlist.")
        return False
    ...
    print(f"   - Archivos descargados: {len(downloaded_files)}")
    print(f"   - Archivos en .trash: {len(trash_files) if trash_folder.exists() else 0}")
    print(f"   - Archivos restantes: {len(remaining_files)}")
    print()
